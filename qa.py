...
if 'logged_in' not in st.session_state:
    ...
    st.session_state.user_id = None # Email for QA, ID for Agent
...
if 'audit_logs' not in st.session_state:
    # Adding 'Auditor Name' to columns to track who performed the audit
...
def login():
    ...
    with col1:
        ...
        if user_role == "QA":
            u_id = st.text_input("Email Address")
        elif user_role == "Agent":
            u_id = st.text_input("Employee ID")
        else:
            u_id = st.text_input("Username") # Admin or others

        password = st.text_input("Password", type="password")
        
        if st.button("Login"):
            # Simple Password Check (Ager motoi '1234' rakha hoyeche)
            if password == "1234":
                if user_role == "Select Role" or not u_id:
                    st.error("Please provide both Role and Credentials!")
                else:
...
if not st.session_state.logged_in:
    ...
    # রোল অনুযায়ী মেনু ফিল্টার (আপনার রিকোয়েস্ট অনুযায়ী)
    ...
    if choice == "Agent Details":
        ...
        with tab1:
            ...
            if edit_id:
                ...
                default_name, default_gender, default_contact, default_emg, default_chan, default_stat = \
                    agent_data['Name'], agent_data['Gender'], agent_data['Contact'], agent_data['Emergency_Contact'], agent_data['Channel'], agent_data['Status']
            ...
            with st.form("agent_form", clear_on_submit=not edit_id):
                ...
                a_chan = col2.selectbox("Assigned Channel", ["Inbound", "Live Chat", "Report Issue", "Complaint Management"], index=["Inbound", "Live Chat", "Report Issue", "Complaint Management"].index(default_chan))
    ...
    # --- ৩. QA অডিট এন্ট্রি (QA Only) ---
        ...
        # QA Name auto-populated from Login Email
        auditor_email = st.session_state.user_id
        
        c_date, c_week, c_qa = st.columns(3)
        c_date.info(f"📅 Date: {current_date}")
        c_week.success(f"🗓️ Week: {week_num}")
        c_qa.warning(f"👤 QA: {auditor_email}")

        st.divider()
        c1, c2, c3 = st.columns(3)
        # Evaluator Name field is now disabled and takes email
        evaluator = c1.text_input("Evaluator Name", value=auditor_email, disabled=True)
        ...
        if params.empty:
            ...
            if st.button("Submit Audit", type="primary"):
                if sel_agent == "Select Agent" or not eval_id:
                    ...
                    entry = {
                        'Week': week_num, 'Evaluation Date': current_date,
